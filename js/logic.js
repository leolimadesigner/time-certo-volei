...
import { db, appId, teamsRef, doc, addDoc, updateDoc, deleteDoc, matchHistoryRef, playersRef } from './firebase.js';
import { showToast, openConfirmModal, closeMoveModal, getTeamName, updateLiveEloPreview } from './ui.js';
        ...
        for (let i = 1; i < eligibleIndices.length; i++) {
            ...
            if (sumCand < sumBest) {
                ...
                for (let j = 0; j < Math.max(sortedBest.length, sortedCand.length); j++) {
                    ...
                    if (cVal < bVal) { candWins = true; break; } 
                    else if (cVal > bVal) { break; }
                }
...
const balanceStrongInside = (playersList, playersPerTeam) => {
    ...
    const waitlistPlayers = sortedPlayers.slice(activePlayersCount).map(p => { let { _rand, ...clean } = p; return { ...clean, waitlistRounds: 0 }; });
...
const balanceStrongOutside = (playersList, playersPerTeam) => {
    const numberOfTeams = Math.floor(playersList.length / playersPerTeam);
    const waitlistSize = playersList.length % playersPerTeam;
    if (numberOfTeams === 0) return { teams: [], waitlist: playersList.map(p => ({...p, waitlistRounds: 0})) };
    const capacities = Array(numberOfTeams).fill(playersPerTeam);
    if (waitlistSize > 0) capacities.push(waitlistSize); 
...
export const redrawTeamWithWaitlist = async (teamId) => {
    openConfirmModal("Sorteio de Substituições", "Deseja substituir este time considerando as prioridades da lista de espera?", async () => {
        ...
        if (pool.length < N) { showToast("Não há jogadores suficientes para formar um time completo.", "warning"); return; }
        if (targetSum === 0) targetSum = pool.reduce((acc, p) => acc + (parseInt(p.categoria) || 1), 0) * (N / pool.length);
        ...
        if (mandatory.length > N) mandatory = mandatory.slice(0, N);
        ...
        let bestCombos = [], minDiff = Infinity, bestSwapCount = -1;
        ...
        if (slotsLeft === 0) {
            ...
            for (const combo of combos) {
                ...
                let isBetter = false, isEqual = false;

                if (diff < minDiff) isBetter = true;
                else if (diff === minDiff) {
                    if (waitlistCount > bestSwapCount) isBetter = true;
                    else if (waitlistCount === bestSwapCount) isEqual = true; 
                }
                if (isBetter) {
                    minDiff = diff; bestSwapCount = waitlistCount; bestCombos = [candidateTeam];
                } else if (isEqual) bestCombos.push(candidateTeam); 
            }
        }
        ...
        try {
            ...
            if (waitlistTeamDoc) {
                if (newWaitlist.length > 0) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistTeamDoc.id), { players: newWaitlist });
                else await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistTeamDoc.id));
...
export const createWaitlist = () => {
    openConfirmModal("Sincronizar Presenças", "Removerá os atletas desmarcados dos times e adicionará os novos na lista de espera. Deseja continuar?", async () => {
        try {
            const waitlistTeamDoc = state.drawnTeams.find(t => t.isWaitlist);
            ...
            const playersInNormalTeamsIds = new Set(
                state.drawnTeams.filter(t => !t.isWaitlist).flatMap(t => t.players.filter(p => state.selectedPlayerIds.has(p.id)).map(p => p.id))
            );
            ...
            if (waitlistTeamDoc) {
                if (updatedWaitlist.length > 0) updatePromises.push(updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistTeamDoc.id), { players: updatedWaitlist }));
                else updatePromises.push(deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistTeamDoc.id)));
        ...
        } catch (e) { showToast("Erro ao atualizar presenças", "error"); }
    });
};
...
export const confirmMovePlayer = async () => {
    ...
    if (!destTeamId || !sourceTeamId || !playerId) { showToast("Erro ao transferir jogador.", "error"); return; }
    ...
    if (destTeam.isWaitlist || sourceTeam.isWaitlist) playerToMove.waitlistRounds = 0;
    ...
    try {
        closeMoveModal();
        const updates = [];
        if (sourceTeam.players.length === 0) updates.push(deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', sourceTeamId)));
        else updates.push(updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', sourceTeamId), { players: sourceTeam.players }));
    ...
    } catch (e) { showToast("Erro ao transferir jogador.", "error"); }
};
...
export const deleteTeam = (id) => {
    openConfirmModal("Remover Equipe", "Deseja remover esta equipe do sorteio?", async () => {
        try { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', id)); showToast("Equipe removida.", "info"); } 
...
export const calculateEloPreview = () => {
    ...
    const eloT1 = getTeamElo(team1), eloT2 = getTeamElo(team2);
    const expectedT1 = 1 / (1 + Math.pow(10, (eloT2 - eloT1) / 400)), expectedT2 = 1 / (1 + Math.pow(10, (eloT1 - eloT2) / 400));
...
export const checkWinCondition = () => {
    ...
    if (isTradicionalWin || isCapoteWin) {
        ...
        const btnSaveResult = document.getElementById('btnSaveResult'), warning = document.getElementById('victoryTeamWarning'), eloInfoDiv = document.getElementById('victoryEloInfo');

        if (!select1.value || !select2.value || select1.value === select2.value) { 
            btnSaveResult.classList.add('hidden'); warning.classList.remove('hidden'); 
            if(eloInfoDiv) eloInfoDiv.classList.add('hidden');
        } else { 
            btnSaveResult.classList.remove('hidden'); warning.classList.add('hidden'); 
            ...
            if (preview && eloInfoDiv) {
                ...
                eloInfoDiv.innerHTML = `<div class="flex justify-between items-center px-2"><span class="text-green-400 font-bold flex items-center gap-1 text-base"><i data-lucide="trending-up" class="w-5 h-5"></i> +${winChange} ELO</span><span class="text-slate-500 font-bold text-[10px] uppercase tracking-widest">Recompensa</span><span class="text-red-400 font-bold flex items-center gap-1 text-base"><i data-lucide="trending-down" class="w-5 h-5"></i> ${loseChange} ELO</span></div>`;
...
export const saveAndCloseVictoryModal = async () => {
    const preview = calculateEloPreview();
    if (!preview) { showToast("Selecione dois times válidos e diferentes no placar!", "error"); return; }
    const { changeT1, changeT2, team1, team2, isTeam1Winner } = preview;
    const actualT1 = isTeam1Winner ? 1 : 0, actualT2 = isTeam1Winner ? 0 : 1;
    ...
    try {
        ...
        await Promise.all(updatePromises);
        showToast(`Ranking Atualizado! Time vencedor faturou +${isTeam1Winner ? changeT1 : changeT2} de Elo.`, "success");
        document.getElementById('victoryModal').classList.add('hidden'); document.getElementById('victoryModal').classList.remove('flex');
        state.score1 = 0; state.score2 = 0; 
        document.getElementById('score1').innerText = state.score1; document.getElementById('score2').innerText = state.score2;
        document.getElementById('team1Select').value = ''; document.getElementById('team2Select').value = ''; 
    } catch (error) { showToast("Erro ao salvar resultado.", "error"); } 
    finally { btnSave.innerText = "SALVAR RANKING"; btnSave.disabled = false; }
};
...
export const promoteWaitlistToTeam = async (waitlistTeamId) => {
    openConfirmModal("Promover Lista de Espera", "Deseja criar um novo time equilibrado usando os jogadores da lista de espera?", async () => {
        ...
        const sizeInput = document.getElementById('teamSize');
        const N = sizeInput ? parseInt(sizeInput.value) || 4 : 4;
        if (waitlistDoc.players.length < N) { showToast(`A lista precisa ter pelo menos ${N} jogadores.`, "warning"); return; }

        const existingTeams = state.drawnTeams.filter(t => !t.isWaitlist);
        let targetSum = existingTeams.length > 0 
            ? existingTeams.reduce((acc, t) => acc + t.players.reduce((sum, p) => sum + (parseInt(p.categoria) || 1), 0), 0) / existingTeams.length 
            : waitlistDoc.players.reduce((acc, p) => acc + (parseInt(p.categoria) || 1), 0) * (N / waitlistDoc.players.length);
        ...
        for (const combo of combos) {
            const diff = Math.abs(combo.reduce((acc, p) => acc + (parseInt(p.categoria) || 1), 0) - targetSum);
            if (diff < minDiff) { minDiff = diff; bestCombos = [combo]; } 
            else if (diff === minDiff) bestCombos.push(combo);
        }
        ...
        const sortedWaitlist = pool.filter(p => !newTeamIds.has(p.id)).sort((a, b) => {
            const catDiff = (parseInt(b.categoria) || 1) - (parseInt(a.categoria) || 1);
            return catDiff !== 0 ? catDiff : a.name.localeCompare(b.name);
        });
        ...
        try {
            ...
            if (existingTeams.length > 0) nextLabelNumber = Math.max(...existingTeams.map(t => parseInt(t.label) || 0)) + 1;
            await addDoc(teamsRef, { label: nextLabelNumber.toString(), players: newTeamPlayers });
            if (sortedWaitlist.length > 0) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistDoc.id), { players: sortedWaitlist });
            else await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'teams', waitlistDoc.id));
            showToast("Nova equipe formada a partir da espera!", "success");
        } catch (e) { showToast("Erro ao promover lista de espera.", "error"); }
    });
};
